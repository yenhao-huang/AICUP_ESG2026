#!/usr/bin/env bash
# submit_11 = submit_9 的 ST1/ST2/ST4 結果不變，僅 ST3 改成：
#
#   ST3 multitask BERT 預測 (stage3/tmp/softvote_raw.csv, source=bert_multitask)
#     -> 對 max-softmax 信心度 < CONF (0.70) 的列，fallback 成已預先算好的
#        codex ST3 標籤 (stage3/tmp/st3_codex_test2000_offsetctx_scoped.csv)
#     -> 用 submit_11 既有的 ST1 (stage1/bert_gemma.csv) + ST2
#        (stage2/softvote_gated.csv) 做 cascade gate -> stage3/softvote_gated.csv
...
#     時才取代；codex 為 N/A / 被自身 gate 過濾的列則保留 multitask 標籤，
#     由本流程的 ST1+ST2 gate 統一決定 N/A。
set -euo pipefail
...
ST3_CODEX="${ST3_CODEX:-$HERE/stage3/tmp/st3_codex_test2000_offsetctx_scoped.csv}"  # 預先算好的 codex 預測 (可用 ST3_CODEX 覆寫成 *_fixed.csv)
