#!/usr/bin/env bash
# add_image on the 100-row set, pinned to the codex backend; prints the Stage 3 score.
# Inputs: --no-add-context --add-image
#   ./run_add_image_100.sh            # codex, 8 workers
#   LIMIT=5 ./run_add_image_100.sh    # quick wiring check on 5 rows
set -euo pipefail
...
LIMIT="${LIMIT:-}"
PROMPT="$EXP/prompts/codex/add-image.txt"
