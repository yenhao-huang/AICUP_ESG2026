#!/usr/bin/env bash
# vanilla on ALL non-N/A rows (val_nonNA, 398), codex backend; prints the Stage 3 score.
# Inputs: --no-add-context  (<data-prompt> only)
# Output: preds/codex/vanilla_nonNA_codex.csv  (does NOT collide with the _100 run)
#   ./run_vanilla_all.sh          # codex, CONC=8
#   CONC=4 ./run_vanilla_all.sh   # lower if codex throttles
#   LIMIT=4 ./run_vanilla_all.sh  # quick wiring check
set -euo pipefail
...
MODEL="${MODEL:-gpt-5.5}"
LIMIT="${LIMIT:-}"
PROMPT="$EXP/prompts/codex/vanilla.txt"
