#!/usr/bin/env bash
# add_page_abstract on the 100-row set, pinned to the codex backend; prints the Stage 3 score.
# Inputs: --add-page-abstract --no-add-context   (<data-prompt> + <page-abstract>)
#   ./run_add_page_abstract_100.sh            # codex
#   CONC=2 ./run_add_page_abstract_100.sh     # codex: lower concurrency to avoid throttling
#   LIMIT=5 ./run_add_page_abstract_100.sh    # quick wiring check on 5 rows
set -euo pipefail
