#!/usr/bin/env bash
# scoped on the 100-row set, pinned to the qwen backend; prints the Stage 3 score.
# Inputs: --add-context --context-mode all
#   ./run_scoped_100.sh            # qwen, 8 workers
#   LIMIT=5 ./run_scoped_100.sh    # quick wiring check on 5 rows
set -euo pipefail
...
LIMIT="${LIMIT:-}"
PROMPT="$EXP/prompts/clear_notclear_with_context_scoped.txt"
