...
# ST2 Gemma fallback (tau=0.9) — summary in stage2/tmp/summary.json; direction via raw-vs-gemma diff
s2 = json.load(open(D / "stage2/tmp/summary.json"))
b2 = {r["id"]: r["evidence_status"] for r in csv.DictReader(open(D / "stage2/tmp/softvote_raw.csv"))}
g2 = {r["id"]: r["evidence_status"] for r in csv.DictReader(open(D / "stage2/tmp/bert_gemma_raw.csv"))}
y2n = sum(1 for i in b2 if i in g2 and b2[i] == "Yes" and g2[i] == "No")
n2y = sum(1 for i in b2 if i in g2 and b2[i] == "No" and g2[i] == "Yes")
w(f"--- fallback_change_rate (Gemma, tau={s2['threshold']}) ---")
w(f"  total rows           {s2['total_rows']:>6}")
w(f"  kept BERT (conf>=tau){s2['kept_bert_rows']:>6}")
w(f"  escalated to Gemma   {s2['escalated_rows']:>6}")
w(f"  changed by Gemma     {s2['changed_by_llm']:>6}  ({100*s2['changed_by_llm']/max(s2['escalated_rows'],1):.1f}% of escalated)")
w(f"  overall change rate  {100*s2['changed_by_llm']/s2['total_rows']:.2f}%")
w(f"    Yes->No            {y2n:>6}")
w(f"    No->Yes            {n2y:>6}")
w()
...
st3c = []
for r in csv.DictReader(open(D / "stage3/softvote_gated.csv")):
