#!/usr/bin/env bash
# submit_9 pipeline = submit_9 (0617 ensemble soft-vote) + Gemma low-conf fallback
#                      injected into Stage1 and Stage2.
#
#   ST1 = soft-vote over the 5 ST1 ensemble members (focal_g3_w4)
#         -> Gemma fallback: rows with max-softmax < ST1_CONF (0.60, ~6.5%)
#            re-predicted by Gemma-4-12B (adapter gemma4_st12_mix)
#   ST2 = soft-vote over the 5 ST2 members
#         -> Gemma fallback: filter_passed rows with max-softmax < ST2_TAU (0.70, ~5.9%)
...
# fallback, because the heads emit ungated labels for every row. Each stage dir
# root holds exactly ONE non-delta csv (merge_pipeline picks it non-recursively);
# raw/ungated intermediates live under stageN/tmp/.
set -euo pipefail
...
ST1_CONF="${ST1_CONF:-0.60}"   # ST1 BERT max-softmax below this -> Gemma (~6.5%, 130 rows)
...
#       Use the fully-fixed local copy (stage4/tmp/stage4_codex_predictions_fixed.csv,
#       == 0615/fix_stage4_predictions/stage4_codex_predictions_fixed.csv): every row
#       carries a concrete timeline (no raw N/A) before the ST1 gate is applied.
ST4_CODEX_RAW="${ST4_CODEX_RAW:-$HERE/stage4/tmp/stage4_codex_predictions_fixed.csv}"
