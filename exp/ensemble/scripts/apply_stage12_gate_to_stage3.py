#!/usr/bin/env python3
"""Apply stage1 + stage2 gate to stage3 predictions.

Stage3 (evidence_quality / clarity) is only applicable when the cascade allows it:

    ST1 promise_status == "Yes"  AND  ST2 evidence_status == "Yes"

Otherwise stage3 is not applicable and must be "N/A":

    - ST1 promise_status == "No"            -> ST2/ST3/ST4 = N/A  -> ST3 = "N/A"
    - ST1 == "Yes" but ST2 != "Yes"         -> ST3 not applicable -> ST3 = "N/A"

This script reads a stage1 CSV (id + promise_status) and a stage2 CSV
(id + evidence_status), then for every row in the stage3 CSV whose id is gated
out by either stage, sets evidence_quality = "N/A".

Usage
-----
    python exp/integrated_stage_predictions/0613/scripts/apply_stage12_gate_to_stage3.py \
        --stage1 .../stage1.csv \
        --stage2 data/predictions/0611/submit_2/stage2/bert.csv \
        --stage3 .../out_test2000_.../stage3_codex_predictions.csv \
        --output .../stage3_codex_gated.csv

    # overwrite in place
    python exp/integrated_stage_predictions/0613/scripts/apply_stage12_gate_to_stage3.py \
        --stage1 .../stage1.csv \
        --stage2 .../stage2.csv \
        --stage3 .../stage3_codex_predictions.csv
"""
...
def read_ids(path: Path, col: str, keep) -> set[str]:
    """Return ids whose `col` value satisfies predicate `keep(value)`."""
    ids: set[str] = set()
...
def main() -> None:
    ...
    # ids gated out by stage1 (promise_status == No)
    st1_no = read_ids(st1_path, args.stage1_col, lambda v: v == "No")
    # ids gated out by stage2 (evidence_status != Yes  -> "No" or "N/A")
    ...
    # Sanity check: no gated id may keep a non-N/A stage3 label
    remaining = sum(
        1 for r in rows
