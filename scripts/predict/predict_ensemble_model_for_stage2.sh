#!/usr/bin/env bash
# Predict Stage 2 labels with the soft-vote ensemble checkpoints under models/.
#
# Run from anywhere:
...
#   output: results/predict/stage2/ensemble/<mode>/softvote.csv
...
PREDICTOR="${PREDICTOR:-core/service/predict/stage2/soft_vote.py}"
...
case "$MODE" in
  submit)
    DEFAULT_MODELS_DIR="models/submission/stage2"
    DEFAULT_RUN_ID="softvote_st2_submit"
    ;;
  local)
    DEFAULT_MODELS_DIR="models/ensemble_models/stage2"
    DEFAULT_RUN_ID="softvote_st2_local"
    ;;
  *)
...
if [ -z "${CKPT_GLOB:-}" ]; then
  case "$MODE" in
    submit) CKPT_GLOB="$MODELS_DIR/*/best_st2.pt" ;;
    local) CKPT_GLOB="$MODELS_DIR/*/seed*/*/best_st2.pt" ;;
  esac
fi
OUTPUT="${OUTPUT:-results/predict/stage2/ensemble/${MODE}/softvote.csv}"
